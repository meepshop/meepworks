...
var _pagination = module.exports = core.createClass({
    ...
    checkSize: function (e)
    {
        ...
        {
            parentRect = this.props.parent.getDOMNode().getBoundingClientRect();
        }
        else
        {
            parentRect = this.getDOMNode().parentNode.getBoundingClientRect();
        }
        /* get bounding rectangle of parent node */
        var btnLimit = Math.floor((parentRect.right - parentRect.left) / 135 - 1);
        /* 
			Assume each button is 45px and subtract 2 buttons for first and last. Adjust to use about 2/3 of parent width.
		*/
        if (btnLimit < 1)
        {
