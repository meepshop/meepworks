...
var _lnListview = {
    propTypes: {
        ...
        lnListviewDisplayButtonTip: propTypes.string
    }
};
var _listview = module.exports = core.createClass({
    ...
    getDefaultProps: function ()
    {
        return {
            ...
            lnListviewDisplayButtonTip: 'Select columns to display',
            rowBody: []
        };
    },
    propTypes: {
    ...
    {
        var displayed = this.state.display.map(function (item)
        {
            return item[0];
        });
        return displayed;
    ...
    setDisplayed: function (display)
    {
        var self = this;
        this.state.display = [];
        display.forEach(function (fieldname)
        {
            for (var i = 0, len = self.props.columns.length; i < len; i++)
            {
                if(self.props.columns[i].fieldname==fieldname)
                {
                    self.state.display.push([fieldname, i]);
                    break;
                }
            }
        });
            ...
            this.props.data.forEach(function (data, idx)
            {
                ...
                if (self.props.rowBody[idx])
                {
                    body.push(_rowBody({
                        key: 'rowbody:' + idx,
                        colSpan: colSpan,
                        children: self.props.rowBody[idx]
                    }));
                }
...
var _extended = _listview.extended = core.createClass({
    ...
    setData: function (data)
    {
        ...
        this.state.rowBody.splice(0, this.state.rowBody.length);
        this.forceUpdate();
    },
    getSelected: function ()
    {
    ...
    getDisplayed: function ()
    {
        return this.refs.listview.getDisplayed();
    },
    setDisplayed: function (display)
    {
        this.refs.listview.setDisplayed(display);
    },
    ...
    {
        this.toggleRowBody(idx);
    },
    toggleRowBody: function (idx)
    {
